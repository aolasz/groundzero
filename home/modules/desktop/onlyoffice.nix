...
in
{
  options.my.desktop.onlyoffice = {
    ...
    cache = {
      enabled = lib.mkEnableOption "local file caching" // {
        default = true;
      };
    ...
    network = {
      ...
      resumableUploads = lib.mkEnableOption "resumable uploads for large files" // {
        default = true;
      };
    };
  };
  ...
  config = lib.mkIf cfg.enable {
    ...
    # Home directory structure - consolidated home.file definition
    home.file = lib.mkMerge [
      # Cache and config directories
      (lib.mkIf cfg.cache.enabled {
      ...
      # Convenience script for launching with SharePoint configuration
      (lib.mkIf cfg.sharepoint.enable {
        "bin/onlyoffice-sharepoint" = {
          executable = true;
          text = ''
            #!/usr/bin/env bash
            ...
            echo "  2. Enter site URL: ${
              if cfg.sharepoint.siteUrl != null then
      ...
      (lib.mkIf cfg.sharepoint.enable {
        ".config/onlyoffice/README-SHAREPOINT.md" = {
          text = ''
